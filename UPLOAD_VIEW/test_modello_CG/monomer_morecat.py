...
##################################################
# CATIONI EXTRA — sparsi nella box
##################################################

n_extra_cations = 500 * len(grouped_quadriplexes)
next_id = max(p.id for p in sim_inst.sys.part) + 1
min_dist = 1.5
rng = np.random.default_rng(seed=sim_inst.seed)

added = 0
max_attempts = 100_000
attempts = 0
if mode_io == "new":
    while added < n_extra_cations and attempts < max_attempts:
        pos = rng.random(3) * sim_inst.sys.box_l
        too_close = False
        for p in sim_inst.sys.part:
            diff = pos - p.pos
            diff -= sim_inst.sys.box_l * np.round(diff / sim_inst.sys.box_l)
            if np.linalg.norm(diff) < min_dist:
                too_close = True
                break
        if not too_close:
            sim_inst.sys.part.add(id=next_id, pos=pos, type=30)
            next_id += 1
            added += 1
        attempts += 1

    if added < n_extra_cations:
        logging.warning(f"⚠️ Solo {added}/{n_extra_cations} cationi extra aggiunti dopo {max_attempts} tentativi.")
    else:
        logging.info(f"✅ {added} cationi extra aggiunti con successo.")
...
sim_inst.sys.non_bonded_inter[30, 30].wca.set_params(
    epsilon=1.0,
    sigma=1.2*2**(-1/6)
)

sim_inst.sys.non_bonded_inter[101,102].morse.set_params(eps=0,alpha=4.8,rmin =0.5*2**(-1/6), cutoff=1.0)

lp=np.sqrt(33/16-np.sqrt(2)/2)

#sim_inst.sys.non_bonded_inter[100,27].morse.set_params(eps=5,alpha=4.8,rmin =lp*2**(-1/6), cutoff=15.0)

#sim_inst.sys.non_bonded_inter[100,30].morse.set_params(eps=5,alpha=4.8,rmin =lp*2**(-1/6), cutoff=15.0)


sim_inst.sys.non_bonded_inter[100,27].lennard_jones.set_params(epsilon=5,sigma=lp*2**(-1/6),cutoff=15,shift="auto")
sim_inst.sys.non_bonded_inter[100,30].lennard_jones.set_params(epsilon=5,sigma=lp*2**(-1/6),cutoff=15,shift="auto")

#electrostatics_solver = DH(prefactor=1.0, kappa=0, r_cut=8.)
...
sim_inst.sys.non_bonded_inter[27, 30].wca.set_params(
    epsilon=1.0,
    sigma=1.2*2**(-1/6)
)
#for multimers
...
############################################
############################################
# REPORT INTERAZIONI ATTIVE
############################################
p############################################
# CHECK RIGOROSO INTERAZIONI ATTIVE
############################################
print("\n" + "!"*60)
print("🔍 VERIFICA REALE DELLE FORZE NEL KERNEL (SOLO ATTIVE)")
print("!"*60)

used_types = sorted({p.type for p in sim_inst.sys.part})

for i in used_types:
    for j in used_types:
        if j < i: continue  # Evita speculari (i,j) e (j,i)
        
        inter = sim_inst.sys.non_bonded_inter[i, j]
        report = []

        # Check WCA: è attiva solo se epsilon > 0
        try:
            p = inter.wca.get_params()
            if p['epsilon'] > 0:
                report.append(f"  [WCA]   eps={p['epsilon']:.2f}, sigma={p['sigma']:.3f}")
        except: pass

        # Check MORSE: è attiva solo se eps > 0 e cutoff > 0
        try:
            p = inter.morse.get_params()
            if p['eps'] > 0 and p['cutoff'] > 0:
                report.append(f"  [MORSE] eps={p['eps']:.2f}, rmin={p['rmin']:.3f}, cut={p['cutoff']:.1f}")
        except: pass

        # Check LJ: è attiva solo se epsilon > 0
        try:
            p = inter.lennard_jones.get_params()
            if p['epsilon'] > 0:
                report.append(f"  [LJ]    eps={p['epsilon']:.2f}, sigma={p['sigma']:.3f}, cut={p['cutoff']:.1f}")
        except: pass

        # Stampa solo se c'è almeno una forza attiva
        if report:
            # Qui cerchiamo di dare un nome ai tipi se possibile
            name_i = f"Tipo {i}"
            name_j = f"Tipo {j}"
            print(f"\n{name_i} <---> {name_j}:")
            for r in report:
