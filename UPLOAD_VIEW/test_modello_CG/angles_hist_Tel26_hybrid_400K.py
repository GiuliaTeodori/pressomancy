...
for ax, group in zip(axes_hist, angle_groups):
    for label, color in zip(group['labels'], colors):
        data = np.array(group['data'][label])
    ...
    ax.set_title(f"{group['title']} — distribuzione angoli")
    ax.legend(fontsize=8)
    ax.grid(True, alpha=0.3)

axes_hist[-1].set_xlabel('Angolo (°)')
fig_hist.suptitle(f'Distribuzione angoli — Tel26 all-atom {run_tag}', fontsize=13)

plt.tight_layout()
plt.savefig(f"{run_tag}/angles_hist_{run_tag}.png", dpi=150)
plt.show()

print(f"Istogramma salvato in {run_tag}/angles_hist_{run_tag}.png")