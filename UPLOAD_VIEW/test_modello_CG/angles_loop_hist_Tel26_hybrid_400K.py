...
plt.tight_layout()
plt.savefig(f"{run_tag}/angles_time_{run_tag}.png", dpi=150)
plt.show()
print(f"Plot angoli nel tempo salvato")

# =========================
# ISTOGRAMMI ANGOLI
# =========================
fig_hist_angles, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 8), sharex=True)

for label, color in zip(angle_labels[:3], colors[:3]):
    data = np.array(angles_data[label])
    ax1.hist(data, bins=50, alpha=0.5, color=color,
...
for label, color in zip(angle_labels[3:], colors[3:]):
    data = np.array(angles_data[label])
    ax2.hist(data, bins=50, alpha=0.5, color=color,
             label=f'{label} (media={np.mean(data):.1f}°)')
ax2.set_ylabel('Frequency')
ax2.set_xlabel('Angle (°)')
ax2.set_title('Angles distribution')
ax2.legend(fontsize=8)
ax2.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig(f"{run_tag}/angles_hist_{run_tag}.png", dpi=150)
plt.show()
print(f"Istogramma angoli salvato")