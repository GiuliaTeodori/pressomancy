...
sim_inst.sys.thermostat.set_langevin(kT=1.345, gamma=gamma_T,
...
sim_inst.sys.non_bonded_inter[101,102].morse.set_params(eps=2.5,alpha=4.8,rmin =0.5*2**(-1/6), cutoff=1.5)

lp=np.sqrt(33/16-np.sqrt(2)/2)
#sim_inst.sys.non_bonded_inter[100,27].morse.set_params(eps=2.5,alpha=4.8,rmin =lp*2**(-1/6), cutoff=5)
...
#      Checkpoint                              #
#############################################################
h5_pathh=f"/home/stekajack/DATA/traiettorie/{context_string}.lammpstrj"
...
if completed_steps==0:
    ...
    fuoir(sim_inst, step=0, box_dim=box_dim,filename=h5_pathh, mode="new")
    ...
    for i in range(n_blocks):
        ...
        fuoir(sim_inst, step=timestep, box_dim=box_dim,filename=h5_pathh, mode="load")
        #checkpoint.save()
    ...
    for sample_i in range(n_samples):
        ...
        #salva_osservabili(timestep,mode='load')
        fuoir(sim_inst, step=timestep, box_dim=box_dim,filename=h5_pathh, mode="load")
        if sample_i==300:  # Salva checkpoint ogni 10 campioni
            checkpoint.save()
            logging.info(f"💾 Checkpoint salvato dopo sample {sample_i + 1}")
        #checkpoint.save()
        GLOBAL_COUNTER += 1
