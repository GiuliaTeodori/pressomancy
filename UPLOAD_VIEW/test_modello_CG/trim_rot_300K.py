...
from utility_sim1 import fuoir, calc_v, find_by_id, salva_osservabili, fuoir_patches_only
import os
import json
import shutil

context_string = "trimer_prova"
...
if mode_io == "new":
    ...
    # ==========================================
    # CATENA ORDINATA PER FILAMENTO
    # ==========================================
    corner_order = {}
    for fil_id, fil in enumerate(telomeres):
        chain = fil.get_full_chain()
        corner_order[fil_id] = [c.id for c in chain]

    order_path = os.path.join(run_dir, "corner_order.json")
    with open(order_path, "w") as f:
        json.dump(corner_order, f, indent=2)
    print(f"✅ Corner order salvato in {order_path}")
    # ==========================================
    # FLAT MAPPING REAL → VIRTUAL
    # ==========================================
    EXCLUDED_TYPES = {24, 25, 27, 100}
    ...
    for fil_id, fil in enumerate(telomeres):
        ...
        for quad_id, quad in enumerate(fil.associated_objects):
            for quartet_id, quartet in enumerate(quad.associated_objects):
                for part_type, plist in quartet.type_part_dict.items():
                    for p in plist:

                        # modifica charged solo nel quartet centrale
                        if quartet_id == 0:
                            circ_type = Quartet.part_types.get('circ')
                            if circ_type is not None and p.type == circ_type:
                                print(f"Particella {p.id} circ -> 400")
                                p.type = 400

                        if int(p.type) in EXCLUDED_TYPES:
                            continue

                        if hasattr(p, "vs_relative"):
                            rel = p.vs_relative
                            if rel is not None and rel[0] != -1:
                                real_id = int(rel[0])
                                virt_id = int(p.id)
                                real_part = sim_inst.sys.part.by_id(real_id)
                                if int(real_part.type) in EXCLUDED_TYPES:
                                    continue
                                if real_id not in flat_map:
                                    flat_map[real_id] = []
                                flat_map[real_id].append(virt_id)

        analysis_data[fil_id] = {
            "ordered_reals": corner_order[fil_id],
            "real_to_virtual": flat_map
        }

    analysis_path = os.path.join(run_dir, "filaments_connection.json")
    with open(analysis_path, "w") as f:
        json.dump(analysis_data, f, indent=2)
    print(f"Filament connections salvato in {analysis_path}")

    # ==========================================
    # LAMMPS PATCHES — solo frame iniziale
    # ==========================================
    patches_path = os.path.join(run_dir, "patches_t0.lammpstrj")
    fuoir_patches_only(sim_inst, step=0, box_dim=box_dim, filename=patches_path)
    print(f"✅ Patches frame iniziale salvato in {patches_path}")
...
sim_inst.sys.non_bonded_inter[28,28].lennard_jones.set_params(epsilon=2,sigma=2,cutoff=1.5*2,shift="auto")
...
# ==========================================
# STAMPA TIPI PARTICELLE
# ==========================================
print("\n===== PARTICELLE NEL SISTEMA =====")
for p in sim_inst.sys.part.all():
    print(f"id={p.id:5d}  type={p.type:4d}  pos=({p.pos[0]:.2f}, {p.pos[1]:.2f}, {p.pos[2]:.2f})")
print(f"Totale particelle: {len(sim_inst.sys.part.all())}")
print("===================================\n")
############################################
# SIMULATION RUN
############################################
