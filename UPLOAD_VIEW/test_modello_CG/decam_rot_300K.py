...
if mode_io == "new":
    ...
    for p in sim_inst.sys.part:
        # skip tipi che non vuoi
        if int(p.type) in EXCLUDED_TYPES:
            continue

        # controlla relazione virtuale
        if hasattr(p, "vs_relative"):
            ...
            if rel is not None and rel[0] != -1:
                ...
                # anche il real deve essere valido
                real_part = sim_inst.sys.part.by_id(real_id)
                if int(real_part.type) in EXCLUDED_TYPES:
                    continue

                virtual_to_real[virt_id] = real_id

                if real_id not in real_to_virtual:
                    real_to_virtual[real_id] = []

                real_to_virtual[real_id].append(virt_id)

    # salva
    mapping_path = os.path.join(run_dir, "real_virtual_map.json")
    ...
    print(f"✅ Mappa real-virtual salvata in {mapping_path}")
##################################################
...
#sim_inst.sys.non_bonded_inter[27, 27].wca.set_params(epsilon=1.0, sigma=1.5 * 2 ** (-1 / 6))
