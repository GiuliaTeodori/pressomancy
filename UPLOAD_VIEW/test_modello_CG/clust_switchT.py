...
context_string = "monomer_400_300"
# Imposta i parametri per il logging
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(levelname)s - %(message)s",
    handlers=[
        logging.FileHandler(f"{context_string}_logging.txt"),  # Nome del file di log
...
dir_path = f"/home/stekajack/DATA/checkpoint/{context_string}"
...
sim_inst.set_steric_custom(
    pairs=[
        ...
        ('charged','squareB'),
        ('charged','real'),
        ('deactivatecharged', 'squareA'),
        ('deactivatecharged', 'squareB'),
        ('deactivatecharged', 'real'),
        ('deactivatecharged', 'circ'),
        ('deactivatecharged', 'charged')
...
sim_inst.sys.thermostat.set_langevin(kT=1.342,gamma=gamma_T,
...
sim_inst.sys.non_bonded_inter[100,27].morse.set_params(eps=3.5,alpha=4.8,rmin =lp*2**(-1/6), cutoff=5)
...
#sim_inst.sys.non_bonded_inter[26,26].morse.set_params(eps=1.25,alpha=4.8,rmin =2.0, cutoff=6)
...
if args.MODE == 'LOAD':
    try:
        ...
        checkpoint.load()
        sim_inst.sys.thermostat.set_langevin(kT=1., gamma=1.0, seed=sim_inst.seed)   
