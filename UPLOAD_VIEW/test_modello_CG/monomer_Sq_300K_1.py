...
base_root = "/home/stekajack/DATA/monomer_Sq_300K/data"
...
if mode_io == "new":
    ...
    for i, quad in enumerate(quadriplex):
        print(f"Applicando dihedrals a quadriplex {i}: {quad.who_am_i}")
        quad.add_dihedrals()
...
if mode_io == "new":
    ...
    h5_path_load = os.path.join(sim_root, args.FROM_CHECKPOINT, "data.h5")
    sim_inst.io_dict['h5_file'] = h5py.File(h5_path_load, 'a')
    sim_inst.io_dict['registered_group_type'] = [TelSeq.__name__]
    sim_inst.io_dict['flat_part_view'][TelSeq.__name__] = list(sim_inst.sys.part)
############################################
#SIMULATION RUN
############################################
