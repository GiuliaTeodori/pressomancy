                ...
                for part_type, plist in quartet.type_part_dict.items():
                    for p in plist:
                        ...
                        if quartet_id == 0:
                            ...
                            if charged_type is not None and p.type == charged_type:
                                print(f"Particella {p.id} charged -> 400")
                                p.type = 400
...
sim_inst.sys.non_bonded_inter[24, 25].morse.set_params(eps=2.4, alpha=4.8, rmin=0.5 * 2 ** (-1 / 6), cutoff=1.5)
...
total_steps = eq_total + n_samples * steps_between_samples

# -------- EQUILIBRAZIONE --------
if current_step < eq_total:
    print("⚖️ Equilibrazione")
    while current_step < eq_total:
        sim_inst.sys.integrator.run(eq_block)
        current_step = round(sim_inst.sys.time / md_timestep)
        checkpoint.save()
        pct = 100 * current_step / eq_total
        print(f"⏱️ Equilibrazione: timestep {current_step}/{eq_total} ({pct:.2f}%)")

# -------- PRODUZIONE --------
print("Produzione")

int_steps_completed = max(0, current_step - eq_total)
...
for sample_i in range(current_sample, n_samples):
    ...
    checkpoint.save()
    save_all(timestep)
    pct = 100 * (timestep - eq_total) / (n_samples * steps_between_samples)
    logging.info(f"Produzione: timestep {timestep} ({pct:.2f}%)")

logging.info("Fine simulazione")